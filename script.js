window.addEventListener('popstate', (event) => {
            ...
            function goPopular() {
                ...
                gallery.innerHTML = '<div style="grid-column: 1 / -1; height: 60vh; display: flex; justify-content: center; align-items: center; color: #888; font-size: 18px; font-weight: 500;">Loading Top Anime...</div>';
            ...
            window.showDetails = function (id) {
                ...
                const searchInputEl = document.getElementById('navbarSearch');
                const dropdownEl = document.getElementById('searchDropdown');

                function getSearchHistory() {
                    const history = localStorage.getItem('anime_search_history');
                ...
                function addToHistory(term) {
                    ...
                    localStorage.setItem('anime_search_history', JSON.stringify(history));
                }
                ...
                function renderLiveResults(query) {
                    ...
                    if (matches.length === 0) {
                        dropdownEl.innerHTML = '<div style="padding:15px; color:#888; text-align:center; font-size:13px;">No results found</div>';
                    ...
                    matches.forEach(anime => {
                        ...
                        div.innerHTML = `
                            <img src="${anime.image}" class="search-thumb" alt="${anime.title}">
                            <div class="search-info">
                                <div class="search-title">${anime.title}</div>
                                <div class="search-meta">
                                    <span class="badge badge-eps">${anime.episodes || '?'} EPS</span>
                                    <span class="badge badge-rating">${anime.rating || 'N/A'}</span>
                                </div>
                            </div>
                        `;
                ...
                function performSearch(query) {
                    ...
                    if (document.body.getAttribute('data-view') !== 'home') {
                        goHome(); 
                    }
                ...
                searchInputEl.addEventListener('focus', () => {
                    if (searchInputEl.value.trim().length > 0) {
                        renderLiveResults(searchInputEl.value.trim());
                    } else {
                        renderHistoryDropdown();
                    }
                });
